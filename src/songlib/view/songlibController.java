...
public class songlibController {
	...
	public void addSong(Event e) 
	{
		...
		if (result.get() == ButtonType.OK){
			...
			if (errorFound(list, songNameField.getText().trim(), songArtistField.getText().trim())) {
				...
				if (songNameField.getText().trim().isEmpty() && songArtistField.getText().trim().isEmpty()) {
	...
	public void editSong(Event e) 
	{
		...
		if (result.get() == ButtonType.OK)
		{
			...
			alert.setHeaderText("400 Error");
			
			Alert emptyListAlert = new Alert(AlertType.ERROR);
			emptyListAlert.setTitle("Edit Song Error Message");
			emptyListAlert.setHeaderText("500 Error");
			
			if (list.isEmpty())
			{
				emptyListAlert.setContentText("Enter a song before editing");
				emptyListAlert.showAndWait();
			}
			else if (errorFound(list, editsongNameField.getText(), editsongArtistField.getText())) 
			{
				alert.setContentText("Song already exists");
				alert.showAndWait();
			} 
